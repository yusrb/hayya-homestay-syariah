...
<div class="max-w-7xl mx-auto animate-slide-in">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
    ...
    <div class="card-hover bg-white rounded-2xl p-6 shadow-soft border border-neutral-200/50">
      ...
      <div class="mt-4 flex items-center text-sm text-accent-amber">
        <i class="fa-solid fa-arrow-up mr-1"></i>
        <span>1 baru minggu ini</span>
      </div>
    </div>
    ...
    <div class="card-hover bg-white rounded-2xl p-6 shadow-soft border border-neutral-200/50">
      ...
      <div class="mt-4 flex items-center text-sm text-emerald-600">
        ...
        <span>Semua terkonfigurasi</span>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    ...
    <div class="glass-effect rounded-2xl p-6 shadow-soft border border-neutral-200/50">
      <h3 class="text-lg font-semibold text-neutral-700 mb-4">Aktivitas Pengunjung</h3>
      <div class="relative h-64">
        @if (empty($visitorData))
          <div class="flex items-center justify-center h-full text-neutral-500">
            <p class="text-sm">Belum ada data pengunjung untuk minggu ini</p>
          </div>
        @else
          ...
          <div class="flex items-start">
            ...
            <div>
              <p class="text-sm font-medium text-neutral-700">
                {{ $activity->user ? $activity->user->name : 'Sistem' }}: {{ $activity->rincian }}
              </p>
  ...
  <div class="glass-effect rounded-2xl p-6 shadow-soft border border-neutral-200/50">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <a href="{{ route('admin.glampings.create') }}" class="flex flex-col items-center justify-center p-4 rounded-lg bg-emerald-50 hover:bg-emerald-100 transition-all card-hover">
      ...
      <a href="{{ route('admin.facilities.create') }}" class="flex flex-col items-center justify-center p-4 rounded-lg bg-emerald-50 hover:bg-emerald-100 transition-all card-hover">
        <div class="w-10 h-10 rounded-full bg-emerald-50 flex items-center justify-center mb-2">
          <i class="fa-solid fa-plus text-emerald-600"></i>
        </div>
        <span class="text-sm font-medium text-emerald-700">Tambah Fasilitas</span>
      </a>
      <a href="{{ route('admin.packages.create') }}" class="flex flex-col items-center justify-center p-4 rounded-lg bg-accent-amber/10 hover:bg-accent-amber/20 transition-all card-hover">
        <div class="w-10 h-10 rounded-full bg-accent-amber/10 flex items-center justify-center mb-2">
          <i class="fa-solid fa-plus text-accent-amber"></i>
        </div>
        <span class="text-sm font-medium text-accent-amber">Tambah Paket</span>
      </a>
      <a href="{{ route('admin.gallery_items.create') }}" class="flex flex-col items-center justify-center p-4 rounded-lg bg-emerald-50 hover:bg-emerald-100 transition-all card-hover">
        <div class="w-10 h-10 rounded-full bg-emerald-50 flex items-center justify-center mb-2">
          <i class="fa-solid fa-plus text-emerald-600"></i>
        </div>
  ...
  @if (!empty($visitorData))
    ...
    const chart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          label: 'Pengunjung',
          data: @json(array_values($visitorData)),
          borderColor: '#14b8a6',
